...
**最終更新**: 2026-09-14（タイトルエスケープ修正の知見を追記）  
...
## 2026-09-14 — 記録スクリプトのタイトルエスケープ

ユーザー入力のタイトルを YAML frontmatter と索引表へ無エスケープで埋め込んでいたため、`|` / `:` / `"` / 改行で構文が壊れた。対応の要点:

- 共通関数 `ckms_yaml_escape` / `ckms_table_escape` を `_skill-base.sh` に集約（jq/python 非依存を維持）
- 表行の `awk` 挿入は `awk -v` ではなく `ENVIRON`（`\|` の `\` が消えるため）
- スラッグ化の前に改行を畳む（`sed` の行分割でファイル名に改行が入るのを防止）
