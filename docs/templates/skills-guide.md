...
```bash
# エントリ追加
bash .claude/skills/knowledge-management/scripts/add-entry.sh "フレームワーク選択"
```
...
```bash
# パターン追加
bash .claude/skills/pattern-library/scripts/add-pattern.sh "API エラーハンドリング"
```
...
```bash
# セッション作成
bash .claude/skills/debug-workflow/scripts/create-session.sh "ログイン画面のエラー"

# 過去セッション検索
bash .claude/skills/debug-workflow/scripts/search-sessions.sh "認証"
```
...
```bash
# 改善エントリ追加
bash .claude/skills/improvement-tracking/scripts/add-improvement.sh "APIレスポンスキャッシュ導入"
```
...
```bash
# 自動セットアップ
bash path/to/templates/.claude/skills/project-setup/scripts/init.sh /path/to/project
