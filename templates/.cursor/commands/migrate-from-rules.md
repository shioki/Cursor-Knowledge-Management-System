...
v2.x の `.cursor/rules` 形式から v3.0.0 の Agent Skills + Commands 形式への移行を対話形式で支援します。
...
まだの場合は、ユーザーにコピー方法を案内してください:

```bash
# Mac/Linux
cp -r path/to/templates/.cursor/skills .cursor/skills
...
| 旧ファイル | 新ファイル |
|-----------|-----------|
| `.cursor/knowledge.md` | `.cursor/skills/knowledge-management/references/KNOWLEDGE_TEMPLATE.md` |
| `.cursor/patterns.md` | `.cursor/skills/pattern-library/references/PATTERNS_TEMPLATE.md` |
| `.cursor/context.md` | `.cursor/skills/project-context/references/CONTEXT_TEMPLATE.md` |
| `.cursor/debug-log.md` | `.cursor/skills/debug-workflow/references/DEBUG_TEMPLATE.md` |
| `.cursor/improvements.md` | `.cursor/skills/improvement-tracking/references/IMPROVEMENTS_TEMPLATE.md` |
...
1. **team-standards.mdc のカスタマイズ**
   → `.cursor/skills/team-standards/SKILL.md` の該当セクションに反映

2. **project-context.mdc のカスタマイズ**
   → `.cursor/skills/project-context/SKILL.md` に反映

3. **独自ルール**
   → 新しいスキルとして `.cursor/skills/` に作成を提案
