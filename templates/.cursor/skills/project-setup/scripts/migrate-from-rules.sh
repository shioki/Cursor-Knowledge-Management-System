#!/usr/bin/env bash
# project-setup: v2.x (.cursor/rules) から v3.0.0 (skills + commands) への移行スクリプト
#
...
TEMPLATES_ROOT="$(cd "$TEMPLATES_CURSOR_DIR/.." && pwd)"
...
if [ -d "$TARGET/.cursor/skills" ]; then
  log_warn "skills/ ディレクトリが既に存在します"
...
# マッピング定義: 旧ファイル → 新ファイル
declare -A DATA_MAP=(
  ["knowledge.md"]="skills/knowledge-management/references/KNOWLEDGE_TEMPLATE.md"
  ["patterns.md"]="skills/pattern-library/references/PATTERNS_TEMPLATE.md"
  ["context.md"]="skills/project-context/references/CONTEXT_TEMPLATE.md"
  ["debug-log.md"]="skills/debug-workflow/references/DEBUG_TEMPLATE.md"
  ["improvements.md"]="skills/improvement-tracking/references/IMPROVEMENTS_TEMPLATE.md"
)
...
echo ""
echo "  新しいスキル:     $(find "$TARGET/.cursor/skills" -name "SKILL.md" | wc -l | tr -d ' ') 個"
...
echo "     .cursor/skills/team-standards/SKILL.md をプロジェクトの規約に更新"
echo ""
echo "  3. 構造を検証してください"
echo "     bash .cursor/skills/project-setup/scripts/validate.sh"
echo ""
