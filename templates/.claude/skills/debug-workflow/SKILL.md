...
デバッグセッションの作成（保存先: `.claude/debug-sessions/` または `.cursor/debug-sessions/`）:

```bash
bash .claude/skills/debug-workflow/scripts/create-session.sh "問題の概要"
# （.cursor/skills にも対応）
```
...
```bash
bash .claude/skills/debug-workflow/scripts/search-sessions.sh "キーワード"
# （.cursor/skills にも対応）
```
