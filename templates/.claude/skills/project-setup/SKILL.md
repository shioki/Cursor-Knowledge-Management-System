...
**デフォルト**: Cursor と Claude Code で共有（`.claude/skills` に配置）
```bash
bash path/to/cursor-knowledge-management-system/templates/.claude/skills/project-setup/scripts/init.sh /path/to/target-project
```

**Cursor のみ**: Claude Code 未使用の場合は `--cursor-only`
```bash
...
```powershell
# Windows (PowerShell)
Copy-Item -Path "templates\.claude\skills" -Destination ".claude\skills" -Recurse
...
4. team-standards スキルの SKILL.md をプロジェクトの規約に更新（`.claude/skills/team-standards/SKILL.md` または `.cursor/skills/team-standards/SKILL.md`）
