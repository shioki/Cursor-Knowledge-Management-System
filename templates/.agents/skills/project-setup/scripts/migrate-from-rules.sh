#!/usr/bin/env bash
# project-setup: v2.x (.cursor/rules) から v5.0.0 (skills + commands) への移行スクリプト
#
# Usage: bash migrate-from-rules.sh /path/to/target-project [--no-confirm] [--legacy-claude|--cursor-only]
#
# 引数:
#   $1 - ターゲットプロジェクトのパス（必須）
#   --no-confirm    - 確認プロンプトをスキップ（任意）
#   --legacy-claude - .claude/skills に配置（v4.x 互換）
#   --cursor-only   - .cursor/skills に配置（Cursor 専用モード）
#
# デフォルト: .agents/skills に配置（Cursor 3.x / Claude Code / Codex 公式互換）
#
# 処理内容:
...
if [ -z "$TARGET" ]; then
  ...
  echo "Usage: bash migrate-from-rules.sh /path/to/target-project [--no-confirm] [--legacy-claude|--cursor-only]"
  exit 1
fi
...
echo ""
echo "========================================="
echo "  v2.x → v5.0.0 マイグレーションツール"
echo "========================================="
echo ""
echo "ターゲット:   $TARGET"
echo "配置モード:   $MODE ($BASE_DIR/skills)"
echo ""
...
if [ -d "$TARGET/$BASE_DIR/skills" ]; then
  log_warn "$BASE_DIR/skills/ ディレクトリが既に存在します"
...
cp -r "$TEMPLATES_AGENTS_DIR/skills" "$TARGET/$BASE_DIR/skills"
log_ok "skills/ を $BASE_DIR/skills にコピーしました"

mkdir -p "$TARGET/$BASE_DIR/debug-sessions"
touch "$TARGET/$BASE_DIR/debug-sessions/.gitkeep" 2>/dev/null || true
...
find "$TARGET/$BASE_DIR/skills" -name "*.sh" -exec chmod +x {} \;
...
declare -A DATA_MAP=(
  ["knowledge.md"]="$BASE_DIR/skills/knowledge-management/references/KNOWLEDGE_TEMPLATE.md"
  ["patterns.md"]="$BASE_DIR/skills/pattern-library/references/PATTERNS_TEMPLATE.md"
  ["context.md"]="$BASE_DIR/skills/project-context/references/CONTEXT_TEMPLATE.md"
  ["debug-log.md"]="$BASE_DIR/skills/debug-workflow/references/DEBUG_TEMPLATE.md"
  ["improvements.md"]="$BASE_DIR/skills/improvement-tracking/references/IMPROVEMENTS_TEMPLATE.md"
)
...
echo ""
echo "  新しいスキル:     $(find "$TARGET/$BASE_DIR/skills" -name "SKILL.md" | wc -l | tr -d ' ') 個"
...
echo "     $BASE_DIR/skills/team-standards/SKILL.md をプロジェクトの規約に更新"
echo ""
echo "  3. 構造を検証してください"
echo "     bash $BASE_DIR/skills/project-setup/scripts/validate.sh"
echo ""
